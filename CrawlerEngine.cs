...
namespace BitcoinCrawlerStats
{
    public class CrawlerEngine
    {
                    ...
                    while (TryParseMessage(msInner, sessionInfo, out var command, out var payload))
                    {
                        ...
                        if (command == "version")
                        {
                            ...
                            if ((services & (int)ServiceFlags.NODE_NETWORK) != 0)
                                ServiceStats.AddOrUpdate(nameof(ServiceFlags.NODE_NETWORK), 1, (_, c) => c + 1);

                            if ((services & (int)ServiceFlags.NODE_NETWORK_LIMITED) != 0)
                                ServiceStats.AddOrUpdate(nameof(ServiceFlags.NODE_NETWORK_LIMITED), 1, (_, c) => c + 1);

                            if ((services & (int)ServiceFlags.NODE_UASF_REDUCED_DATA) != 0)
                            {
                                sessionInfo.SessionHistory.HasBip110 = true;
                                ServiceStats.AddOrUpdate(nameof(ServiceFlags.NODE_UASF_REDUCED_DATA), 1, (_, c) => c + 1);
                            }
        ...
        private static string ParseUserAgent(byte[] payload, out ulong services)
        {
            ...
            foreach (var kvp in Sessions)
            {
                ...
                if (_settings.DisableEvaluation)
                {
                    if (sessionAge.TotalSeconds > 30)    // Ought to be enough for anyone to handshake, right???
                    {
                        ...
                        si.SessionHistory.Active = active;
