...
namespace BitcoinCrawlerStats
{
    public class CrawlerPersistence
    {
        ...
        public async Task LoadFromDbAsync()
        {
            ...
            // DISABLED: let's not load/save protocol and service stats, because they would become erroneous when program is restarted
            //await LoadUserAgentsTable<ProtocolInfo>(_engine.ProtocolStats);
            //await LoadUserAgentsTable<ServiceInfo>(_engine.ServiceStats);

            await LoadHostsTable(_db.Evaluated, _engine.Evaluated);
            await LoadPeersTable(_db.Unvisited, _engine.Unvisited);
            await LoadSessionHistory(_engine.AllSessionHistory);
        }
        ...
        public async Task ResetDatabaseAsync()
        {
            ...
            await _db.Evaluated.ExecuteDeleteAsync();
            await _db.ProtocolStats.ExecuteDeleteAsync();
            await _db.ServiceStats.ExecuteDeleteAsync();
            await _db.SessionHistory.ExecuteDeleteAsync();
            await _db.Unvisited.ExecuteDeleteAsync();
        }
        ...
        async Task PersistDataAsync()
        {
            ...
            //                    async () => await PersistUserAgentsTable<ProtocolInfo>(_engine.ProtocolStats));
            //await RunAndHandleError("ServiceStats",
            //                    async () => await PersistUserAgentsTable<ServiceInfo>(_engine.ServiceStats));
        ...
        async Task<int> PersistUserAgentsTable<T>(IReadOnlyDictionary<String, int> dict) where T : class, IGenericStatisticRecord
        {
            ...
            var dictCopy = dict.ToDictionary(p => p.Key, p => new { Id = p.Key, Count = p.Value }); // Use a copy to avoid concurrent changes...

            // First update existing...
            foreach (var kvp in dictCopy.Where(p => dbUserAgents.ContainsKey(p.Key) && dbUserAgents[p.Key].Count != p.Value.Count))
            {
                var entity = dbUserAgents[kvp.Key];
                _db.Entry(entity).CurrentValues.SetValues(dictCopy[kvp.Key]);
            }
            ...
            var newUserAgents = dictCopy
                                    ...
                                    .Select(p =>
                                    {
                                        var instance = (T)Activator.CreateInstance(typeof(T), p.Key, p.Value.Count)!;
                                        return instance;
                                    })
                                    .ToList();
        ...
        async Task LoadSessionHistory(IDictionary<String, SessionHistory> dict)
        {
        ...
        async Task PersistSessionHistory(ConcurrentDictionary<String, SessionHistory> dict)
        {
            ...
            // First update existing...
            foreach (var kvp in dictCopy.Where(p => dbEntries.ContainsKey(p.Key) && !p.Value.Ignore))
            {
                var entity = dbEntries[kvp.Key];
                _db.Entry(entity).CurrentValues.SetValues(kvp.Value);
            }
